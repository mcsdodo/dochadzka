...
1. **First visit** → "Download sample file" button. User saves `dochadzka.json`, edits config in text editor.
...
4. **Auto-save** → Every edit saves to file immediately (debounced 500ms).

### Browser support

- Supported: Chrome, Edge, Opera (File System Access API)
- Unsupported: Firefox, Safari (show warning message)

## UI Layout

```
┌─────────────────────────────────────────────────────────┐
│  [Reconnect to dochadzka.json]  [Open file]  [Signature]│  ← Top bar
├─────────────────────────────────────────────────────────┤
│  ◀ 12-2025    [01-2026]    02-2026 ▶    [+ New month]   │  ← Month nav
├─────────────────────────────────────────────────────────┤
...
│  SPOLU                                           104     │
│                                                          │
│  D - dovolenka, S - sviatok, PN - práceneschopnosť...   │
│  Prestávka* - prestávka na odpočinok a jedenie §91 ZP...│
│                                                          │
...
### Hidden in print
- Top bar (file buttons, signature button)
- Month navigation
- Cell focus indicator

### Styling
- Black text on white background (except dark blue headline)
...
- `downloadSampleFile()` - creates and downloads template JSON
- `openFile()` - File System Access API file picker
- `saveFile()` - write data to file handle (debounced)
- `generateMonth(year, month)` - creates default month data
- `calculateEaster(year)` - returns Easter Sunday date
...
4. Implement sample file download
